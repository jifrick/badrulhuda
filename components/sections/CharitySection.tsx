    ...
    <section id="charity" className="relative py-28 px-6 md:px-8 overflow-hidden bg-surface-alt border-t border-b border-border/40 dark:border-border/10">
      ...
      {isModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          {/* Modal Container */}
          <div className="relative bg-white dark:bg-surface border border-accent/25 dark:border-accent/15 w-full max-w-sm rounded-3xl p-6 sm:p-8 shadow-2xl flex flex-col items-center gap-6 z-10 text-center animate-in fade-in zoom-in-95 duration-200">
            {/* Premium inset gold border frame with corner accents */}
            <div className="absolute inset-2 border border-accent/10 rounded-[20px] pointer-events-none">
              <div className="absolute top-1 left-1 w-2 h-2 border-t border-l border-accent/30" />
              <div className="absolute top-1 right-1 w-2 h-2 border-t border-r border-accent/30" />
              <div className="absolute bottom-1 left-1 w-2 h-2 border-b border-l border-accent/30" />
              <div className="absolute bottom-1 right-1 w-2 h-2 border-b border-r border-accent/30" />
            </div>

            {/* Header */}
            <div>
              <h4 className="font-amiri text-base tracking-[0.15em] font-medium text-accent uppercase">
                Select Payment App
              </h4>
              <p className="text-[11px] text-textColor-secondary dark:text-textColor-muted font-inter mt-2 max-w-[240px] mx-auto leading-relaxed">
                Choose your preferred app to complete the transaction directly.
              </p>
            </div>
            ...
            <div className="flex flex-col gap-3 w-full">
              ...
              <a
                ...
                onClick={() => setIsModalOpen(false)}
                className="w-full inline-flex items-center justify-center gap-2.5 bg-[#1A1A1A] hover:bg-black text-white font-bold py-2.5 px-4 rounded-xl shadow-md transition-all duration-300 active:scale-95 text-xs font-inter uppercase tracking-wider border border-neutral-800"
              >
                <svg className="w-4 h-4 bg-white rounded-full p-0.5 shrink-0" viewBox="0 0 24 24">
                  <path
                    fill="#4285F4"
              ...
              <a
                ...
                onClick={() => setIsModalOpen(false)}
                className="w-full inline-flex items-center justify-center gap-2.5 bg-[#25D366] hover:bg-[#20ba5a] text-white font-bold py-2.5 px-4 rounded-xl shadow-md transition-all duration-300 active:scale-95 text-xs font-inter uppercase tracking-wider"
              >
            ...
            <button
              onClick={() => setIsModalOpen(false)}
              className="text-xs font-semibold text-textColor-muted hover:text-textColor-primary transition-colors focus:outline-none mt-1"
            >
              Cancel
            </button>
          </div>

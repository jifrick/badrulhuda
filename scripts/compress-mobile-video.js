...
const inputPath = path.join(__dirname, "..", "public", "videos", "WhatsApp Video 2026-06-24 at 10.06.17 AM.mp4");
const outputDir = path.join(__dirname, "..", "public", "videos");
const outputPath = path.join(outputDir, "hero-bg-mobile.mp4");
...
async function main() {
  ...
  // -vf "scale='min(720,iw)':-2" : Scale width to 720px for mobile devices to keep file size tiny!
  const cmd = `"${ffmpeg}" -y -i "${inputPath}" -an -vcodec libx264 -crf 28 -preset medium -vf "scale='min(720,iw)':-2" "${outputPath}"`;
